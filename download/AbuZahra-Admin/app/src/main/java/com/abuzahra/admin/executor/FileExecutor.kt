...
object FileExecutor {
    ...
    fun listFiles(context: Context, params: Map<String, Any>): List<Map<String, Any>> {
        ...
        if (dir.exists() && dir.isDirectory) {
            ...
            list.add(mapOf("error" to "Directory not found: $dirPath"))
        }
        return list
    }
    ...
    fun getFileInfo(context: Context, params: Map<String, Any>): Map<String, Any> {
        ...
        return if (path.isNotBlank()) {
            try {
                ...
                } else mapOf("error" to "File not found")
            } catch (e: Exception) { mapOf("error" to e.message ?: "") }
        } else mapOf("error" to "No path")
    }
